...
### v0.3.1 (2026-02-08) - 验证修复

**修复**
- 管理端云养/捐献页面：改用 `ApiClient.request()` 替代错误的 `API.get()` 调用
- curl 测试用例：登录 API 改为 query 参数方式（后端 `@RequestParam` 不接受 JSON body）
- curl 测试用例：`/donation/my` 修正为 `/donation/my-donations`

**验证通过**
- 用户端：登录 → 云养列表 → 发起云养 → 我的云养 → 捐献项目 → 发起捐献 → 我的捐献 → 打赏猫咪
- 管理端：登录 → 猫咪管理 → 领养申请 → 众筹管理 → 云养管理 → 捐献管理
